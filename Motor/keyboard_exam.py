...
# PWM 설정
dc_motor_pwm = GPIO.PMW(dc_motor_pwm_pin, 1000) # 1kHz 주파수의 PMW 생성
servo = GPIO.PWM(servo_pin, 50) # 서보 모터용 50Hz 주파수의 PWM 생성
dc_motor_pwm.start(0) # DC 모터의 초기 속도 0%
servo.start(0) # 서보 모터의 초기 위치 0%
...
def set_servo_angle(angle):
    ...
    if 0 <= angle <= 180: # 각도가 서보 모터와 허용 범위 내에 있는 경우
