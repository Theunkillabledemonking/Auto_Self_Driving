...
try:
    ...
    while True:
        ...
        if keyboard.is_pressed('up'):
            print("Up key pressed: DC Motor Forward")
            set_dc_motor(50, "forward")
        # 아래쪽 방향키를 누르면 DC 모터 후진
        elif keyboard.is_pressed('down'):
            print("Down key pressed: DC Motor Backward")
            set_dc_motor(50, "backward")
        # 방향키가 눌리지 않으면 DC 모터 멈춤
        else:
            dc_motor_pwm.ChangeDutyCycle(0)  # Stop motor if no key is pressed

        # 왼쪽 방향키를 누르면 서보 모터를 좌측으로 회전
        if keyboard.is_pressed('left'):
            print("Left key pressed: Servo Left")
            set_servo_angle(current_servo_angle - servo_speed)
        # 오른쪽 방향키를 누르면 서보 모터를 우측으로 회전
        elif keyboard.is_pressed('right'):
            print("Right key pressed: Servo Right")
            set_servo_angle(current_servo_angle + servo_speed)
